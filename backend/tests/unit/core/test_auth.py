...
def test_create_access_token(auth_service: AuthService) -> None:
    ...
    token = auth_service.create_access_token(user_id, tenant_id, "test@example.com")
    assert isinstance(token, str)
    assert len(token) > 0


def test_verify_token_roundtrip(auth_service: AuthService) -> None:
...
def test_verify_token_expired_raises(auth_service: AuthService) -> None:
...
def test_verify_token_invalid_signature_raises(auth_service: AuthService) -> None:
...
def test_verify_token_missing_claims_raises(auth_service: AuthService) -> None:
...
def test_create_token_pair(auth_service: AuthService) -> None:
    """create_token_pair returns both access and refresh tokens."""
    user_id = uuid_utils.uuid7()
    tenant_id = uuid_utils.uuid7()
    pair = auth_service.create_token_pair(user_id, tenant_id, "test@example.com")
...
def test_refresh_token(auth_service: AuthService) -> None:
    """Refreshing a token returns a new valid token pair."""
    user_id = uuid_utils.uuid7()
    tenant_id = uuid_utils.uuid7()
    pair = auth_service.create_token_pair(user_id, tenant_id, "test@example.com")

    new_pair = auth_service.refresh_token(pair.refresh_token)
    assert new_pair.access_token
    # Verify the new access token is valid
    claims = auth_service.verify_token(new_pair.access_token)
    assert claims.sub == str(user_id)
